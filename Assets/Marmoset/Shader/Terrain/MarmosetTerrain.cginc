...
struct Input {
	...
	float3 worldNormal;
	#if defined(MARMO_DIFFUSE_FRESNEL) || defined(MARMO_SPECULAR) || defined(MARMO_SPECULAR_IBL)
		float3 viewDir;
	#endif
	INTERNAL_DATA
};
...
void MarmosetTerrainSurf (Input IN, inout SurfaceOutput OUT) {
	...
	//SPECULAR & FRESNEL FADE
			...
			#ifdef MARMO_NORMALMAP
				float3 worldE = WorldNormalVector( IN, -localE );
			#else
				float3 worldE = localE;
			#endif
	#endif

	#if defined(MARMO_DIFFUSE_FRESNEL) || defined(MARMO_SPECULAR_IBL)
		half fade = IN.texcoord.z;
		fade = saturate((fade - _FadeNear) / max(0.0001, _FadeRange));
		fade = 1.0-fade; //TODO: can we get rid of this 1-?
	#endif

	//SPECULAR
	#ifdef MARMO_SPECULAR_IBL
		float3 R = reflect(worldE,worldN);
		#ifdef MARMO_SKY_BLEND
			float3 R1 = R;
