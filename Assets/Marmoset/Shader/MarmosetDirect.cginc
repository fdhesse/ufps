...
inline half4 LightingMarmosetDirect( MarmosetOutput s, half3 viewDir, UnityGI gi ) {
	...
	fixed4 c;
#ifdef MARMO_DIFFUSE_MIN_COLOR
	//c = marmosetLighting (s, viewDir, gi.light.dir, max(_MinColor.rgb, gi.light.color));
	...
	c = marmosetLighting (s, viewDir, gi.light.dir, gi.light.color);
		...
		#ifdef MARMO_DIFFUSE_MIN_COLOR
			c.rgb += s.Albedo * (_MinColor.rgb - min(_MinColor.rgb, gi.indirect.diffuse + c.rgb)); //只增加暗部的亮度
