...
public class SocketClient {
    ...
    void OnConnect(IAsyncResult asr) {
        ...
        readOp = client.GetStream().BeginRead(byteBuffer, 0, MAX_READ, new AsyncCallback(OnRead), null);
    ...
    void OnReceive(byte[] bytes, int length) {
        ...
        while (RemainingBytes() > 2) {
            ...
            var messageLen = reader.ReadUInt32();
            if (RemainingBytes() >= messageLen) {
                MemoryStream ms = new MemoryStream();
                BinaryWriter writer = new BinaryWriter(ms);
                var data = reader.ReadBytes((int)messageLen);
                writer.Write(data);
    ...
    public void Close() {
        if (client != null) {
            if (client.Connected)
            {
                //client.GetStream().EndRead(readOp);
                client.Close();
            }
            client = null;
        }
