...
public class vp_MPPlayerSpawner : Photon.MonoBehaviour
{
	...
	public static void InstantiatePlayerPrefab(PhotonPlayer player, ExitGames.Client.Photon.Hashtable playerStats)
	{
		...
		if (player.ID == PhotonNetwork.player.ID)
		{
            ...
            VariableModifierGUI helper = newPlayer.GetComponentInChildren<VariableModifierGUI>();
            if (helper == null)
                helper = newPlayer.AddComponent<VariableModifierGUI>();
            if( helper != null )
            {
                vp_FPController ctrl = newPlayer.GetComponentInChildren<vp_FPController>();
                vp_FPInputMobile input = newPlayer.GetComponentInChildren<vp_FPInputMobile>();
                vp_FPWeaponHandler weapon = newPlayer.GetComponentInChildren<vp_FPWeaponHandler>();
                helper.FPController = ctrl;
                helper.InputMobile = input;
                helper.WeaponHandler = weapon;
                helper.MinWidthCaption = 100;
                helper.MinWidthValue = 70;
                helper.MinheightField = 30;
                helper.GUIScale = 2;
            }


            FPOperationManager operationMgr = newPlayer.GetComponentInChildren<FPOperationManager>();
            if (operationMgr == null)
                operationMgr = newPlayer.AddComponent<FPOperationManager>();
            if (operationMgr != null)
            {
            ...
            var controlPrefab = (GameObject)vp_MPPlayerStats.GetFromHashtable(playerStats, "Control");
            if (controlPrefab != null)
            {
                var control = Instantiate(controlPrefab);
                control.GetComponent<vp_UIManager>().Player = newPlayer.GetComponent<vp_FPPlayerEventHandler>();
            }
		}
		else
		{
