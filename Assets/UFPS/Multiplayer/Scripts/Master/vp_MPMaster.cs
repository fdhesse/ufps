...
public class vp_MPMaster : Photon.MonoBehaviour
{
	...
	protected virtual void OnEnable()
	{
        ...
        vp_GlobalEvent<SpawnInfo>.Register("SpawnMonster", OnSpawnMonster);
        vp_GlobalEvent<bool>.Register("NetEndGame", OnEndGame);

        //about operation object
        vp_GlobalEvent<int, int, int>.Register("C2MChangeOperationState", C2MChangeOperationObjState);
        vp_GlobalEvent<OperationStateChangeParam>.Register("M2CChangeOperationState", M2CChangeOperationObjState);
        vp_GlobalEvent<OperationStateChangeParam>.Register("M2CChangeOperationStateImm", M2CChangeOperationObjStateImm);

        //about supply mode rule
	...
	protected virtual void OnDisable()
	{
        ...
        vp_GlobalEvent<SpawnInfo>.Unregister("SpawnMonster", OnSpawnMonster);
        vp_GlobalEvent<bool>.Unregister("NetEndGame", OnEndGame);
        vp_GlobalEvent<int, int, int>.Unregister("C2MChangeOperationState", C2MChangeOperationObjState);
        vp_GlobalEvent<OperationStateChangeParam>.Unregister("M2CChangeOperationState", M2CChangeOperationObjState);
        vp_GlobalEvent<OperationStateChangeParam>.Unregister("M2CChangeOperationStateImm", M2CChangeOperationObjStateImm);
    ...
    {
        photonView.RPC("NetEndGame", PhotonTargets.All, win);   
    }

    private void OnSpawnMonster(SpawnInfo info)
    {
        photonView.RPC("SpawnMonster", PhotonTargets.All, info.id, info.path, info.pos, info.quat);   
    }
    ...
    [PunRPC]
    public void SpawnMonster(int id, string path, Vector3 pos, Quaternion quat)
    {
        var rm = AppFacade.Instance.GetManager<LuaFramework.ResourceManager>(LuaFramework.ManagerName.Resource);
        rm.LoadPrefab("monsters", path, (objs) =>
        {
            if (objs[0])
            {
                var monster = (GameObject)UnityEngine.Object.Instantiate(objs[0], pos, quat);
                NetworkMonster.Init(monster);
                MonsterManager.Instance.AddMonster(id, monster);
            }
        });
    }

    //C端通知M端有状态改变了
    public void C2MChangeOperationObjState(int playerID, int ObjectID, int stateID)
    {
        ...
        photonView.RPC("MReceiveChangeOperationObjState", PhotonTargets.MasterClient, playerID, ObjectID, stateID );   
    }



    public void M2CChangeOperationObjState( OperationStateChangeParam changeParam )
    {
        if( !vp_Gameplay.IsMaster )
        {
            return;
    ...
    [PunRPC]
    public void MReceiveChangeOperationObjState(int playerID, int ObjectID, int stateID, PhotonMessageInfo info)
    {
        //试着改变当前状态
        OperationObject.eChangeOperationObjectStateResult result = OperationObject.TryChangeState( playerID, ObjectID, stateID);
        ...
        changeParam.ObjectID = ObjectID;

        if (obj == null)
        {
            changeParam.ToStateID = stateID;
            changeParam.StateTime = 0.0f;
            changeParam.SavePercent = 0.0f;
            changeParam.Result = (int)OperationObject.eChangeOperationObjectStateResult.ECOOSR_ERROR_INVALID_OBJECT;
            M2CChangeOperationObjState(changeParam);
        }
        else if( result != OperationObject.eChangeOperationObjectStateResult.ECOOSR_SUCCESS )
        {
            changeParam.ToStateID = (int)obj.GetCurState();
            changeParam.StateTime = obj.GetCurStateTime();
            changeParam.SavePercent = 0.0f;
            changeParam.Result = (int)result;

            M2CChangeOperationObjState(changeParam);
        }
        else
        {
            obj.RefreshMC( playerID );
        }
    ...
    public void M2CSetExitZoneVisble( bool visible )
    {
        photonView.RPC("CSetExitZoneVisible", PhotonTargets.All, visible );   
    }


    //告诉C玩家是否出现 撤退点
    [PunRPC]
    public void CSetExitZoneVisible( bool visible )
    {
        vp_GlobalEvent<bool>.Send("SetExitZoneVisble", visible);
    }


    public void M2CCoopSupplyModeFinish(bool isSuccess)
    {
        photonView.RPC("CCoopSupplyModeFinish", PhotonTargets.All, isSuccess);  
    }

    //告诉C玩家游戏已经结束
    [PunRPC]
    public void CCoopSupplyModeFinish(bool isSuccess)
    {
