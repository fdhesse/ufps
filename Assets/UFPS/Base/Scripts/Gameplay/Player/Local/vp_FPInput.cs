...
public class vp_FPInput : vp_Component
{
    ...
    public bool IsOnlyInteractInput = false;
	...
	protected override void Update()
	{
		...
		// interaction
        InputInteract();
        // from InputOperation.txt
        if (IsOnlyInteractInput)
        {
            return;
        }
	...
	protected virtual void InputCrouch()
	{
		...
		if (vp_Input.GetButton("Crouch") /*|| UnityEngine.Input.GetButton("Fire1")*/)
...
// when developing for GearVR. please report if it causes any trouble
	    ...
	    if (null != AccuracyController)
	    {
	        m_CurrentMouseLook.x *= AccuracyController.AimHelperCurrentFriction;
	        m_CurrentMouseLook.y *= AccuracyController.AimHelperCurrentFriction;
	        //m_CurrentMouseLook += AccuracyController.ADSFocusEnemyHelper();
