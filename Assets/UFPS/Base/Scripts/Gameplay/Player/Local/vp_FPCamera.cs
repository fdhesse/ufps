...
{
	// magnet state  -- add by hxh
    enum  eFPCameraMagnetState
    {
        //自由状态
        ECMS_FREE,
    ...
    // accuracy controller of the parent gameobject
    public vp_FPAccuracyController AccuracyController;
	...
	// magnent state and state time  -- add by hxh
    private eFPCameraMagnetState mMagnetState = eFPCameraMagnetState.ECMS_FREE;
    private float mMagnetStateTime = 0.0f;
    ...
    private bool _ADSSnap;

    // Magnetism parameters
    private GameObject _lastMagnetismEnemyTracked;
    private float _magnetismTrackingStarted;
	...
	protected override void Awake()
	{
	    ...
	    AccuracyController = Root.GetComponent<vp_FPAccuracyController>();
	...
	protected override void LateUpdate()
	{
		...
		TryCameraCollision();
		// fixed by hxh
        PerformCameraRotations2();
		//from tamaya
	    //PerformCameraRotations();
		// third person
		Update3rdPerson();
	}
	
	// add by hxh
    private void PerformCameraRotations2()
    {
        ...
        Parent.rotation = vp_MathUtility.NaNSafeQuaternion(xQuaternion, Parent.rotation);

        // pitch and yaw the camera
        Quaternion yQuaternion = Quaternion.AngleAxis(-m_Pitch, Vector3.left);
        Transform.rotation = vp_MathUtility.NaNSafeQuaternion(xQuaternion * yQuaternion, Transform.rotation);

        // roll the camera
        Transform.localEulerAngles += vp_MathUtility.NaNSafeVector3(Vector3.forward * m_RotationSpring.State.z);        
    }
	
    private void PerformCameraRotations()
    {
        // rotate the parent gameobject (i.e. player model)
        // NOTE: this rotation does not pitch the player model, it only applies yaw
...
/* from tamaya
    ...
    {
        var ce = AccuracyController.ClosestEnemy;
        var dst = ce == null ? 
            float.MaxValue : 
            new Ray(Transform.position, Transform.forward).DistanceToLine(ce.transform.position);

        if (dst > AccuracyController.MagnetismLockDistance)
        {
            _lastMagnetismEnemyTracked = null;
            return;
        }
        if (ce != _lastMagnetismEnemyTracked)
            _magnetismTrackingStarted = Time.time;
        _lastMagnetismEnemyTracked = ce;

        if (Time.time - _magnetismTrackingStarted > AccuracyController.MagnetismDuration) return;

        Vector2 relRotation = GetEnemyYawPitch();

        m_Yaw += relRotation.x * Time.deltaTime * AccuracyController.ADSSnapSpeed;
        m_Pitch -= relRotation.y * Time.deltaTime * AccuracyController.ADSSnapSpeed;
    }
*/
    private void Magnetism()
    {
        ...
        {
            _lastMagnetismEnemyTracked = null;
            mMagnetState = eFPCameraMagnetState.ECMS_FREE;
            //Debug.Log( "dst > AccuracyController.MagnetismLockDistance : " + mMagnetState.ToString() + Time.renderedFrameCount.ToString());
            return;
        }
        ...
        if (Time.time - _magnetismTrackingStarted > AccuracyController.MagnetismDuration)
        {
            mMagnetState = eFPCameraMagnetState.ECMS_FREE;
            //Debug.Log("time out : " + mMagnetState.ToString() + Time.renderedFrameCount.ToString());
            return;
        } 
        else
        {
            //Debug.Log("timing!!!!! : " + (Time.time - _magnetismTrackingStarted).ToString() );
        }
        Vector2 relRotation = GetEnemyYawPitch();
    ...
    private void ADSSnap()
    {
        if (m_Player.Zoom.Active)
        {
            ...
            var dst = ce == null ? 
                float.MaxValue : 
                new Ray(Transform.position, Transform.forward).DistanceToLine(ce.transform.position);
            if (dst > AccuracyController.ADSSnapDistance || dst < .01f)
                ce = null;
            _ADSSnap = ce != null && (_ADSSnap || !_lastZoomState);
        }
        if (!m_Player.Zoom.Active)
            _ADSSnap = false;

        _lastZoomState = m_Player.Zoom.Active;

        if (!_ADSSnap) return;
        Vector2 relRotation = GetEnemyYawPitch();

        if (relRotation.magnitude > 1)
...
*/
    ...
    private void ADSSnap2()
    {
        bool isZoomStateChanged = false;
        if (_lastZoomState != m_Player.Zoom.Active)
        {
            isZoomStateChanged = true;
            _lastZoomState = m_Player.Zoom.Active;
        }

        if( mMagnetState == eFPCameraMagnetState.ECMS_FREE )
        {
            if( isZoomStateChanged && m_Player.Zoom.Active )
            {
                var ce = AccuracyController.ClosestEnemy;
                var dst = float.MaxValue;
                if (ce != null)
                {
                    dst = new Ray(Transform.position, Transform.forward).DistanceToLine(ce.transform.position);
                }

                if (dst < AccuracyController.ADSSnapDistance)
                {
                    mMagnetState = eFPCameraMagnetState.ECMS_AIM;
                }

                mMagnetState = eFPCameraMagnetState.ECMS_AIM;
                mMagnetStateTime = 0.0f;
            }
        }
        else if (mMagnetState == eFPCameraMagnetState.ECMS_AIM)
        {
            var ce = AccuracyController.ClosestEnemy;

            mMagnetStateTime += Time.deltaTime;
            if( ce == null )
            {
                mMagnetState = eFPCameraMagnetState.ECMS_FREE;
                mMagnetStateTime = 0.0f;
            }
            else if( mMagnetStateTime > AccuracyController.MagnetismDuration )
            {
                mMagnetState = eFPCameraMagnetState.ECMS_FREE;
                mMagnetStateTime = 0.0f;
            }
            else
            {
                ...
                {
                    _lastMagnetismEnemyTracked = null;
                    mMagnetState = eFPCameraMagnetState.ECMS_LOCK;
                    mMagnetStateTime = 0.0f;
                }
                else
                {
        ...
        else if( mMagnetState == eFPCameraMagnetState.ECMS_LOCK )
        {
            //Debug.LogError( "Is In Lock State" );
            Magnetism();
        }
    }


    private Vector2 GetEnemyYawPitch()
    {
        Vector2 res = Vector2.zero;

        if (null == AccuracyController || AccuracyController.ClosestEnemy == null) return res;


        var enemyPosition = AccuracyController.ClosestEnemy.transform.position;
        var groundedEnemyPosition = enemyPosition;
        groundedEnemyPosition.y = Parent.position.y;

        res.x = Vector3.Angle(groundedEnemyPosition - Parent.position, Parent.forward) *
                (Vector3.Dot(groundedEnemyPosition - Parent.position, Parent.right) > 0 ? 1 : -1);

        var f = Quaternion.AngleAxis(res.x, Vector3.up) * Transform.forward;
        var enemyVector = (enemyPosition - Transform.position).normalized;
        res.y = Vector3.Angle(enemyVector, f) * (enemyVector.y - f.y > 0 ? 1 : -1);
