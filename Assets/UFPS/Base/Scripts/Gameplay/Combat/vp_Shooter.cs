...
{
    public static Action<Vector3> FireSoundTriggered;
    ...
    public float MaxProjectileSpreadMove = 10f;         // maximal accuracy deviation added regarding each unit of movement (clamped at vp_FPController.MaxPlayerVelocity) 
    public float MaxProjectileSpreadRotate = 15f;       // maximal accuracy deviation added regarding each unity of rotation (clamped at vp_FPController.MaxPlayerRotationAngle)
    public float MaxProjectileSpreadROF = 15f;          // maximal accuract deviation added regarding the time since the last fire occured
    public float MaxROFAffectTime;                      // maximal time during when a fire affect the accuracy
    ...
    private List<float> _fireTimes = new List<float>(); 
	...
	public GameObject m_MuzzleFlash = null;			// the instantiated muzzle flash. one per weapon that's always there
    ...
    public float[] LastFiresDeltaTime
    {
        get { return _fireTimes.Select(ft => Time.time - ft).ToArray(); }
    }
	...
	protected override void Awake()
	{
        ...
        if (null != m_CharacterController)
    	    _accuracyController = m_CharacterController.GetComponent<vp_FPAccuracyController>();
	...
	{
	    _fireTimes.Add(Time.time);
	    while (Time.time - _fireTimes[0] > MaxROFAffectTime)
	        _fireTimes.RemoveAt(0);
		// update firing rate
	...
	public void SetSpread(int seed, Transform target)
	{
		...
		//--add by hxh
		if( _accuracyController == null )
        {
            target.Rotate(0, Random.Range(-ProjectileSpread, ProjectileSpread), 0);
        }
        else
        {
            target.Rotate(0, Random.Range(-_accuracyController.CurrentAccuracy / 2f, _accuracyController.CurrentAccuracy / 2f), 0);		// then rotate around y with user defined deviation
        }		
	}
